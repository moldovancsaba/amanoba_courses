...
- **Tasklist** (`docs/archive/tasklists/...`): executable checklist + “what’s next” + the **exact next command**.
...
- Tasklists (checkboxes + exact next command): `docs/archive/tasklists/`
...
- Tasklist: `docs/archive/tasklists/<CCS_ID or COURSE_ID>__<timestamp>.md`
...
- **Structure**: follow `docs/layout-grammar.md` lesson grammar (clear intro → main content → recap → action items).
- **Source transparency**: each lesson must include all 3 blocks:
  1) all actually used sources,
  2) all actually checked-but-not-used sources with reason,
  3) non-exhaustive further reading list with multiple source types.
- **Tool recommendation clarity**: when a lesson recommends tools, include 2-3 concrete options with direct links.
...
- **No unresolved deictics** (no vague pronoun/deictic shorthand in the target language unless the exact referent is named in the same sentence).
...
Quiz SSOT: `docs/reference/quiz-quality-pipeline-handover.md` and `docs/reference/quiz-quality-pipeline-playbook.md`.

---

## 4) The end-to-end workflow (Idea → Outline → CCS → Build → QA → Publish)

This section is a condensed “how to do it”; the full checklist is in `docs/reference/course-creation-qa-playbook.md`.
...
### Phase F.1 — Mandatory day-by-day production loop (no batching)
For each day `N` (1..30), follow this exact order:
1) Start day `N`.
2) Build `lesson-N`, then run full lesson quality assurance for that lesson.
3) Build the related `lesson-N-quiz` (7 questions with answers, standalone stems, no lesson references), then run full quiz quality assurance for that quiz.
4) Close day `N` in run log and tasklist, then continue to day `N+1`.

Hard stop rule:
- Never open day `N+1` if day `N` is not marked closed with PASS/FAIL and documented fixes.
...
- Or follow `docs/reference/QUIZ_ITEM_QA_HANDOVER.md` (+ `.state/quiz_item_qa_state.json` if using the cursor-based loop).
...
The app connects using `dbName: process.env.DB_NAME || 'amanoba'` (see `docs/course-building-rules.md` and `app/lib/mongodb.ts`).
...
## Process State
- Status: **RUNNING**
- Tasklist: `docs/archive/tasklists/<MATCHING_TASKLIST>.md`
- Current phase: <A/B/C/...>
...
### 8.2 Tasklist template (`docs/archive/tasklists/...`)
```md
...
- `docs/archive/delivery/2026-01/2026-01-27_RAPID_CHILDREN_COURSES_ACTION_PLAN_AND_HANDOVER.md` (course hierarchy: CCS → language variants → shorts; draft/publish implications)
